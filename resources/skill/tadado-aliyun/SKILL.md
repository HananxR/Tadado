...
## 步骤 3：生成分享链接（**分享文件夹，不是单个文件**）

```powershell
.\aliyunpan.exe share set -mode 1 /Tadado
# → 链接：https://www.alipan.com/s/xxxxxxxx   提取码：xxxx
```

两种模式：`-mode 1` 普通分享（**只支持少数文件类型**，`.exe` 与文件夹实测都可以）、
`-mode 3` 快传链接（支持大部分文件，zip 之类走这条）。**先试 `-mode 1`**，失败再换 `-mode 3`。

### ⚠️ 一定要分享**文件夹**，别分享那个 exe

踩过一次：执行 `share set -mode 1 /Tadado/Tadado2_setup_v1.0.0.exe` —— 它**也能成功**，
但生成的链接打开**只看得到那一个文件**，用户会以为「别的版本都没了」，
而你从云盘这边看（`ls /Tadado`）明明 32 个文件都在。**分享对象不同，看到的东西就不同。**

分享**文件夹**的好处：

- 链接**长期不变** —— README 写一次就够，发版不用回来改；
- 用户能看到**全部版本**（含 v0.x 的历史包），要翻旧版也找得到。

代价：用户进去得自己挑版本。所以 README 里要**点明该拿哪一个**
（写「认准 `Tadado2_setup_v<版本>.exe`」），并说明旧版（`Tadado_*`，v0.x / PySide6）**已停止更新**。

⚠️ 它默认生成的是**私密分享（带提取码）**。这是好事，但 **README 里必须把提取码写上** ——
不写等于没分享。

⚠️ 云盘的分享本身**写不了描述文字**，所以「哪个是新的、哪个已停更」这类备注只能写在 README 里。
