...
## 📋 功能对照遗留（原型对比缺失清单，待用户圈定保留/砍掉 — 未自动执行）

- [ ] Markdown 实时 HTML 预览（编辑面板）
- [ ] 草稿模式（新建预填 + 未保存提示横幅）
- [ ] 多任务创建对话框
- [ ] 截止快速计算器（6 选项弹窗）
- [ ] ConfirmBar 二次确认（批量删除/清除）
- [ ] 管理页多条件筛选侧栏（状态/时间/进度/标签/归档）
- [ ] 分区密码 / 空闲锁定 / 归档天数设置
- [ ] 标签重命名 / 合并真实交互（现为占位）
- [ ] 活动报告内容搜索框
- [ ] 系统托盘菜单细节 + 每日摘要气泡
...
- [ ] 数据现在由前端直连 SQLite，**没有 Rust 侧业务命令**（`lib.rs` 只有 `app_exit`）。要不要把查询收进 Rust，取决于将来是否要接入原版那套 CLI / 命名管道（单一写者）
...
- [ ] 原版还有几项没搬：Markdown 实时预览、草稿模式（未保存提示横幅）、截止快速计算器、分区密码与空闲锁定
- [ ] 工程化：已有 `npm run e2e`（Playwright 冒烟，见 desktop/README），还差 lint / format script 与 CI（`.github/` 已随 Python 版归档，需新建只跑桌面端的流水线）
- [ ] 演示数据随真实时钟后移会「变旧」（它们是按 09-12 排的）。接真实数据时 `mock.ts` 整体删掉即可，不用单独修
- [ ] 设置面板圈范围：现只有主题 / 置顶 / 时间轴粒度三项生真效，其余显示为 `—`
- [ ] 清掉写死的演示口径：总览「较昨日 +2」（`overview.ts` 注明了等历史快照）、活动分析页「导出」只弹 toast「（演示）」
- [ ] 工程化：`package.json` 没有 lint / format / test script，`desktop/` 不在 CI 覆盖内

修正一处清单自身的过期：上面「功能对照遗留」里的**标签重命名 / 合并真实交互**并非未做 ——
桌面版已经实现（`desktop/src/pages/manage.ts`：改名成已存在的标签即视为合并）。圈定
保留 / 砍掉时别再按「待做」处理。
