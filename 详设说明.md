...
```
┌──────────────────────────────────────────────────────────────┐
│ [☐全选] 更改状态 ▾ │ [删除] [中止] [重启]  已选 3 项       │ ← BatchToolbar
├────┬───┬──────────┬──────────────────┬────────┬────┬──────┬──┤
...
- 统计卡片同行右侧显示
- PeriodSelectorBar：昨天/今天/本周/本月 + 自定义日期范围（CalendarPopup）
- 左侧 TaskTreePanel：纯标签列表（无子任务），选中项 accent 色左侧边框
- 右侧 ActivityContentView：有序列表格式（`1. **标题** [状态, 进度]:`）+ 活动条目换行缩进
- 搜索框实时过滤 + 导出（MD/Excel/TXT，默认文件名含分区+日期范围，Excel 分列）
- 时段选择 + 标签点击 + 搜索联动
- 热力图日期点击 → 设置自定义日期范围
...
- `TaskTreePanel`：QListWidget 纯标签 + `tag_selected` 信号
- `ActivityContentView`：QTextBrowser HTML 有序列表渲染 + `set_search_text`/`get_plain_text`
- 导出默认文件名 `{分区}_{日期范围}.{ext}`，Excel 分列（序号/任务/状态变更/进度变更/活动信息）
...
- `_on_import` / `_on_export` 复用 MarkdownImporter/Exporter
...
- 自定义标题栏(36px)：App 图标 + 6 个图标文字按钮（新建单任务/多任务/活动分析/任务管理/设置/帮助▾）+ 右侧图标按钮（缩小托盘/切换全屏/关闭）
...
- `_setup_custom_title_bar()` — 固定 36px QWidget，QHBoxLayout：AppIcon → 6×图标按钮 → stretch → 3×窗口按钮
...
- 全选/取消全选按钮
- 4 个操作按钮：更改状态(进行中/已完成)、删除、中止、重启
- 选中计数标签："已选 N 项"（纯显示，无交互，位于操作按钮之后）
- 无选中时 hover 显示 Toast 提示
- 操作后刷新所有关联视图
- 编辑视图批处理操作通过 QMessageBox 确认，批量视图通过内联确认栏确认

#### 实现方案

- BatchToolbar(QWidget) — 水平布局：全选按钮 + 更改状态下拉 + 删除/中止/重启按钮 + 计数标签
...
- 编辑视图批处理用 QMessageBox 确认（`_confirm_bar` 仅在批量视图中可见）
