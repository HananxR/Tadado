...
- 统计格式：`逾期(X个)、进行中(X个)、待办(X个)、已完成(X个)，共计X个`
...
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│ 📁 工作 :: 逾期(3个)、进行中(2个)、待办(8个)、已完成(5个)，共计18个 | 今日无事 🌿  │ 2026年05月30日 02:30:25 PM │
└─────────────────────────────────────────────────────────────────────────────────────┘
...
**需求**：4 个时段按钮(昨天/今天/本周/本月)，点击后按活动日志数量排名，高亮选中按钮。

**实现方案**：通过 `_get_period_range()` 计算日期区间，`progress_filter_activated(TaskFilter)` 发射。可被 QuickOverviewBar 联动。
