...
- 左侧 TaskTreePanel：可勾选标签列表（默认全选），选中项 accent 色左侧边框，顶部"全选/取消全选"按钮
- 右侧 ActivityContentView：有序列表格式（`1. **标题** [状态, 进度]:`）+ 活动条目换行缩进；滚动到底自动衔接到下一个勾选标签
- 搜索框实时过滤 + 导出（MD/Excel/TXT，按勾选标签全量导出，文件名含分区+日期范围+标签数，Excel 分列）
- 时段选择 + 标签点击/勾选 + 搜索 + 滚动自动衔接联动
- 热力图日期点击 → 设置自定义日期范围
...
- `TaskTreePanel`：QListWidget 可勾选标签（默认全选）+ `tag_selected`/`checked_tags_changed` 信号 + 全选切换按钮 + `get_checked_tags()`/`get_next_checked_tag()`/`select_tag()`/`toggle_all_checked()`
- `ActivityContentView`：QTextBrowser HTML 有序列表渲染 + `set_search_text`/`get_plain_text` + `scrolled_to_bottom` 信号（200ms 防抖）+ 内容加载自动置顶
