...
- 三模块架构之一（活动看板+工作报告+批量处理），Ctrl+2 切换
- 紧凑热力图（12px 单元格，3px 月间隙）+ 悬浮 Tooltip + 点击日期选中
- 4 统计卡片：总活动数/活跃天数/最长连续/日均
- PeriodSelectorBar 时段选择器：昨天/今天/本周/本月 + 自定义日期范围
- ProgressOverviewList 进度总览：标签分组可折叠 + 任务进度卡片（状态点+标题+进度条+活动摘要+报告跳转箭头）
- 点击任务卡片箭头 → 切换到工作报告模块定位该任务

#### 实现方案

- 复用 `CalendarHeatmapWidget`（紧凑常量调整）、`HeatmapStatsPanel`、`HeatmapTooltip`
- 新增 `PeriodSelectorBar`：4 个 checkable QPushButton + 2 个 QDateEdit，emit `period_changed(date_from, date_to, label)`
- 新增 `ProgressOverviewList`：QScrollArea + 标签分组 `_TagHeader` + 任务进度 `_TaskCard`，按时段内活动数筛选排序
- 热力图网格新增 `date_clicked` 信号 + `mousePressEvent`

### 2.7 工作报告（Reports）

**文件**：[src/ui/calendar_heatmap/activity_report_panel.py](src/ui/calendar_heatmap/activity_report_panel.py)（重写）

#### 需求

- Ctrl+3 切换，默认选中"今天"生成日报
- 复用 PeriodSelectorBar 时段选择
- HTML 渲染报告（QTextBrowser）：摘要卡片 + 标签分组 + 任务条目（进度条+时间线）+ 尾部统计
- 搜索过滤：标题/内容匹配，不匹配淡隐
- 导出 Markdown / Excel

#### 实现方案

- 移除旧 QTreeWidget+QTextBrowser 分栏，改为全宽 HTML 渲染
- 新增 API：`show_hint()`（空状态）、`show_task_detail(task, date_from, date_to)`（单任务详情）
- 数据提取 `_extract_report_data` 保持原有逻辑
