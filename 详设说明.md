...
- 合并原"活动看板"和"工作报告"，Ctrl+2 切换，进入时自动选中"今天"
- 紧凑热力图（12px 单元格）+ 悬浮 Tooltip + 点击日期选中
- 统计卡片与 PeriodSelectorBar 同行紧凑布局
- 时段选择器：昨天/今天/本周/本月 + 自定义日期范围，统一控制热力图高亮和报告内容
- HTML 渲染报告（QTextBrowser）：摘要卡片 + 标签分组 + 任务条目（进度条+时间线）+ 搜索过滤 + MD/Excel 导出
- 底部摘要：最活跃任务 + 无活动任务

#### 实现方案

- 复用 `CalendarHeatmapWidget`（紧凑常量）、`HeatmapStatsPanel`、`HeatmapTooltip`、`PeriodSelectorBar`
- `ActivityReportPanel`：QTextBrowser HTML 渲染，`refresh()`/`show_hint()`/`show_task_detail()` API
- 统一 `_on_analysis_period_changed` handler：时段变更 → 热力图高亮 + 报告刷新
- 热力图网格 `date_clicked` 信号 → 设置自定义日期范围
