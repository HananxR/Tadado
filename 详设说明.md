...
- Ctrl+3 切换，左右分栏：左侧管理面板(180px) + 右侧全宽任务表格
- 左侧面板：关键词搜索框 + 归档/清理操作按钮
...
- 左侧面板 `_manage_sidebar`（QWidget, fixedWidth=180）：QVBoxLayout 排列搜索区 + 清理区
- 搜索框 `_manage_search`（QLineEdit + 300ms 防抖），仅关键词搜索，无状态/排序下拉
