...
| 区块 | 配置项 | 控件 |
|------|--------|------|
| 外观 | 主题、最小化到托盘、自动锁定 | 下拉 / 复选框 / 下拉(1/5/10/30/60分钟) |
| 任务列表 | 每页条数、默认排序、热力图起始年份 | 下拉(20/50/100) / 下拉 / 输入框(1900-2200) |
| 提醒 | 启用提醒、间隔(小时)、安静时段 | 复选框 / 输入框(HH:MM) |
| 归档 | 分区设定表格 | 5列：名称\|默认分区\|可见\|自动归档\|归档阈值(天) |
| — | 表格操作 | 右键菜单：设为默认、重命名、删除 |
| 激励语 | 4条激励语 | 单列文本输入 |
...
- 默认分区 QCheckBox 单选互斥（`_on_default_toggled`）
- 删除分区校验：`count_tasks_in_partition()` > 0 时阻止
