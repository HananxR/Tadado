...
**实现方案**：`_show_latest_activity()` 扫描 activity_log 找最新记录。`_rank_tasks()` 按活动日志数降序排列。`set_synced_period()` 联动速览栏按钮启用状态。点击发射 `progress_filter_activated(TaskFilter)` 过滤任务列表。
