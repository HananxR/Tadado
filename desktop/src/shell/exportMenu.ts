// ─────────────────────────────────────────────────────────────────────────────
// 导出按钮：点开选格式（md / txt / xlsx），选完立刻落文件。
//
  ...
  button.addEventListener("click", (event) => {
    ...
    for (const spec of EXPORT_FORMATS) {
      ...
      item.addEventListener("click", (event) => {
        ...
        if (file.bytes) downloadBlob(file.name, new Blob([file.bytes], { type: file.mime }));
        else downloadText({ name: file.name, mime: file.mime, text: file.text ?? "" });
        toast(`已导出 ${options.countText()} → ${file.name}`);
      });
      menu.append(item);
    }
