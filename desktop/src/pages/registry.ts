 ...
 * 页头右侧的一个动作。
 *
 * 这里原来是一个字符串（`action?: string`）—— 一页一个主按钮。任务页需要**两个**
 * 新建入口（单条 / 批量）之后，那条契约就挡住了：第二个只能退回工具行，和搜索、
 * 筛选挤在一起，读起来像「对已有任务做批量操作」，作者本人后来都忘了它是个新建入口。
 * 改成数组，新建入口就都能待在同一个地方。
 */
export interface PageAction {
  ...
  id: string;
  label: string;
  /** 一页最多一个 primary（缺省即次要按钮）。 */
  kind?: "primary" | "secondary";
  /** 文案短、但意思不显然时补一句 tooltip。 */
  title?: string;
}
...
export const PAGES: PageSpec[] = [
  {
    ...
    title: "总览",
    // 「今日指标」改「指标卡」（2026-09-21）：那几张里只有一张说的是「今天」，写成
    // 「今日指标」等于把另外几张说小了。同日晚些又加了第五张「归档」，于是连数量也
    // 不写了 —— 写死的数字一改就得回来改这行
    desc: "指标卡 · 焦点时间轴 · 近期活动 · 优先级分布",
  },
  {
    id: "tasks",
    label: "任务",
    ...
    title: "任务",
    // 页头只留单条新建。「批量新建任务」搬去了任务管理页、并改名「数据迁入」——
    // 它更常被用来导一批旧数据（按分区导出 → 转换 → 迁入），和那页的「导出 ▾」
    // 正好是一进一出；留在任务页页头反而挤占主力动作的位置。
    actions: [{ id: "create", label: "＋ 新建任务", kind: "primary" }],
    // 这行原来是一串**操作说明书**（「单击选中 · 双击打开抽屉 · 右键处置 · 批量迁入在
    // 任务管理页」）—— 那是 Py 版的写法：既没说出这一页是什么，还把别页的入口写进来。
    // 页头说明该回答「这里能看到什么」，操作手势交给悬浮提示（2026-09-21 用户提的）
    desc: "甘特时间轴：条 = 创建 → 截止那一段时间 · 颜色只编码状态 · 按住拖动可平移",
  },
  {
    id: "graph",
    label: "任务图谱",
  ...
  },
  {
    ...
    title: "任务管理",
    // 「数据迁入」挪到这里之后，这一页的说明里得有它 —— 任务页那行原来替它指路
    // （「批量迁入在任务管理页」），现在指路的话交给这一页自己说（2026-09-21）
    desc: "批量审视与处置 · 导出 / 数据迁入 · 行点击打开任务 · 标签改名与合并",
  },
];
